...
export default function Map() {
  ...
  const selectedProvince = useConfigStore((s) => s.selectedProvince);
  ...
  return (
    <CanvasWrapper>
      <Canvas camera={{ fov: 70, position: [0, 20, 10] }} dpr={[1, 2]}>
        <fog attach="fog" args={["#000000", 10, 50]} />
        <color attach="background" args={["#000000"]} />
        <Lights />
        <Suspense fallback={null}>
          <Base key={baseKey} depth={1} data={mapData} outlineData={outlineData} />
        </Suspense>
        <Bottom />
        <Mirror />
        <BeamLight />
